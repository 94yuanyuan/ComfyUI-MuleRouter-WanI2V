...
本節點的核心參數設置是基於官方 Wan 2.6 Image-To-Video 文件 所設計。
...
輸入影像: 新增一個 Load Image (載入影像) 節點，將其 IMAGE 輸出端連接到此節點的 image 輸入端。

輸入提示詞: 在此節點的 prompt (正向提示詞) 與 negative_prompt (負向提示詞) 欄位填寫你想要的畫面描述。

輸入音頻: 若想加入背景音，新增 Load Audio 節點，將其 AUDIO 輸出端連接到此節點左側的 audio 接入點。

輸出預覽: 此節點生成完畢後，會自動將影片 mp4 檔案存至 ComfyUI/output/mulerouter_videos 目錄下。你也可以將此節點輸出的 VIDEO_PATH (字串) 連接給外部的預覽節點或直接到資料夾查看。
...
git clone [https://github.com/94yuanyuan/ComfyUI-MuleRouter-WanI2V.git](https://github.com/94yuanyuan/ComfyUI-MuleRouter-WanI2V.git)
...
將解壓縮出來的整個資料夾放進 ComfyUI/custom_nodes/ 目錄下。
